...
import {fetchArtistConnectData} from '../actions';
        ...
        if (artistData.length === 0) {
            ...
            return (
                <CacheableImage 
                    ...
                    source={{ uri: artistData[0].picture }}
                    resizeMode='cover' >
                    <LinearGradient 
                            ...
                            <View style={styles.learnMorePush}>
                                ...
                                <View style={styles.articleTitle}>
                                    <Text style={styles.titleText}>
                                        {artistData[0].articleTitle}
                                    </Text>
                                </View>

                                <View style={styles.articleDesc}>
                                    <Text style={styles.descText}>
                                        {artistData[0].articleDescription}
                                    </Text>
                                </View>
                                
                                <View style={styles.articleLinks}>  
                                    <TouchableOpacity style={styles.readMore} onPress={() => {
                                        Linking.openURL(artistData[0].articleLink)
                                    }}>
                                    ...
                                    <TouchableOpacity style={styles.connect} onPress={() => {
                                        Linking.openURL(artistData[0].socialLink)
                                    }}>
...
const mapStateToProps = state => {
    return {
        fetching: state.artistData.fetching,
        fetched: state.artistData.fetched,
        error: state.artistData.error,
        artistData: state.artistData.artistData
    }
}

export default connect(mapStateToProps, { fetchArtistConnectData })(LearnMore);